...
int main()
{
    ...
    while(1)
    {
        ...
        for(int i = 0; i < nfds; ++i)
        {
            ...
            {
                epoll_task et = *((epoll_task *)events[i].data.ptr);

                //构造任务
                task t;
                t.sock_fd = et.sock_fd;
                t.sock_fd2 = et.sock_fd2;
                if(et.sock_fd == et.sock_fd2)
                {
                    t.handle_type = 0; //把操作类型设为向下层发送
                }
                else
                {
                    t.handle_type = 1;  //把操作类型设为向上层发送
                }

                //通过互斥锁把任务放到任务队列里
                pthread_mutex_lock(&mutex);
                task_queue_in.push(t);  //把任务放到任务队列里面
